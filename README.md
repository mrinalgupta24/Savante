# Savante

Savante is a Brand and Influencer Matching AI Platform. We are solving the biggest problems of the industry, mainly “matching the right brands and influencers”.

### Installation

1. Clone the repository:
    ```bash
    git clone https://github.com/mrinalgupta24/Savante.git
    ```

2. Install dependencies:
    ```bash
    npm i
    ```

3. Start the application:
    ```bash
    npm run dev
    ```

## Usage
 
Once the development server is running, you can access the app by visiting https://savante-rw5j.vercel.app/ or http://localhost:3000 in your web browser. Here's how to use the app:

1. Click on the "Home" button to access the home page of the website.
2. Click on the "Login" button to navigate to the login page.
3. Click on the "Join Us" button to be directed to the sign-up page.
